...
input_filepath = os.path.join(base_dir, 'all_atm_ondotori20230428.csv')

output_filename = 'all_atm_ondotori{}.csv'.format(dt_now_str)
...
input_last_time = float(input_last_line[1].replace('\"', ''))
...
if len(sys.argv) > 1:
    ...
    plus_first_time = float(plus_first_line[1].replace('\"', ''))
    ...
    if input_last_time >= plus_first_time:
        for i in range(len(plus_f_data)):
            ...
            plus_time = float(plus_line[1].replace('\"', ''))
